...
Loon 配置：
[Script]
http-request ^https:\/\/api\.wetalkapp\.com\/app\/queryBalanceAndBonus script-path=WeTalk_loon.js, tag=WeTalk捕获, enable=true
cron "20 8,20 * * *" script-path=WeTalk_loon.js, tag=WeTalk签到, enable=true
...
// ─── Loon HTTP 请求包装器：将 callback 风格的 $httpClient 转为 Promise ───
function httpFetch(params) {
  return new Promise((resolve, reject) => {
    ...
    const opts = { url: params.url, headers: params.headers };
    ...
    else if (method === 'post') $httpClient.post(opts, callback);
    else if (method === 'put') $httpClient.put(opts, callback);
    else if (method === 'delete') $httpClient.delete(opts, callback);
    else if (method === 'head') $httpClient.head(opts, callback);
    else if (method === 'patch') $httpClient.patch(opts, callback);
...
function runAccount(acc, index, total) {
  ...
  function fetchApi(path) {
    return httpFetch({ url: buildUrl(path, acc.capture), method: 'GET', headers });
  }
